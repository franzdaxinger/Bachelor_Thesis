...
data_inhabitants = pd.read_excel (r'shapefiles/Gemeinden.xlsx.xls', sheet_name='Schweiz - Gemeinden',
                      usecols="A,C", skiprows = 8)
data_inhabitants.columns = ['ID_Gem','inhabitants']
print(data_inhabitants.head())
...
print(array_data)
municipalities = gpd.read_file('shapefiles/municipalities.shp')
...
# save as txt files in shapefiles
with open('shapefiles/diff.txt', 'w') as filehandle:
    for listitem in diffusion:
        filehandle.write('%s\n' % listitem)

with open('shapefiles/rho.txt', 'w') as filehandle:
