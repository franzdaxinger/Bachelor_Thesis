...
rho = 0.001                              # safety factor
tol = 1000.0                              # tolerance of the l2 norm
...
array_alpha = np.genfromtxt('data/alpha.txt')
array_beta = np.genfromtxt('data/beta.txt')
...
while t < T:
    ...
    dt_new = pow(rho * tol / error, 0.5) * deltat
    if dt_new < toldt:
        dt_new = toldt
    ...
    if (error < tol) or (dt_new == toldt):
        ...
        while (math.floor(newtime) - math.floor(t)) > 0:
            SEI_plot = project((SEI_high - SEI_n) * (math.floor(t) + 1.0 - oldtime) / deltat + SEI_n, V)
